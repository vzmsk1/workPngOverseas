...
$(document).ready(function () {
  ...
  let tabId = "0";
  let tabMap = new Map();
  const infoHeight = $('.rigs-design__tab-info[data-rig-design="0"]').css(
    "height",
  );
  $(".rigs-design__tabs-display").css("height", `${toRem(infoHeight)}rem`);

  const changeTabInfo = (item, id) => {
    $(`.rigs-design__tab-item[data-rig-design="${tabId}"]`)[0].classList.remove(
      "active",
    );
    $(`.rigs-design__tab-info[data-rig-design="${tabId}"]`)[0].classList.remove(
      "active",
    );
  ...
  function resizeTabDisplay() {
    if (screen.width < 769) {
      ...
      $(".rigs-design__tabs-display").data("platform", "mobile");
      const infoHeight = $(
        `.rigs-design__tab-info[data-rig-design="${tabId}"]`,
      ).css("height");
      $(".rigs-design__tabs-display").css("height", `${toRem(infoHeight)}rem`);
    } else {
